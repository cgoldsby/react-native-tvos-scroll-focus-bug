...
const Button: FunctionComponent<PressableProps & {text: string}> = props => {
  return (
    <Pressable
      ...
      {({pressed, focused}) => (
        <View style={[styles.button, focused && {backgroundColor: '#EF233C'}, !(props.isTVSelectable ?? true) && {backgroundColor: 'transparent'}, props.style]}>
          <Text style={[styles.buttonText, focused && {color: '#EDF2F4'}, !(props.isTVSelectable ?? true) && {color: '#EDF2F4'}]}>
            {props.text}
          </Text>
        </View>
      )}
...
const App = () => {
  ...
  const [longTextEnabled, setLongTextEnabled] = useState(true);
  const [horizontalContent, setHorizontalContent] = useState(false);
  const [headerFocusable, setHeaderFocusable] = useState(false);
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.listContainer}>
        ...
        />
        <Button
          text={(headerFocusable ? ' ' : 'No ') + 'Focusable Boundaries'}
          onPress={onHeaderFocusable}
        />
        <Button
          text={'Reset'}
          onPress={() => {
            ...
            setHorizontalContent(false);
            setHeaderFocusable(false);
          }}
        />
      </View>
      <View style={styles.textContainer}>
        <Button text={'Header'} isTVSelectable={headerFocusable} />
        ...
        <View style={{flex: 1, width:'100%'}}>
          <TVTextScrollView
            ...
            <Text style={styles.text}>
              {(!longTextEnabled && horizontalContent) ? 'Hello, world!' : longText.slice(0, longTextEnabled ? longText.length : 400) +
                '\n\nThe end.'}
            </Text>
          </TVTextScrollView>
        </View>
